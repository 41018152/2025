...
const NailCareTracker = () => {
  ...
  const toggleDailyTask = async (date, taskKey) => {
    ...
    // 發送到後端保存
    try {
      await fetch('https://nail-care-api.onrender.com/api/daily-tasks', {
        method: 'POST',
        headers: {
  ...
  const toggleWeeklyTask = async (taskKey) => {
    ...
    // 發送到後端保存
    try {
      await fetch('https://nail-care-api.onrender.com/api/weekly-tasks', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const updateManicureTime = async (field, value) => {
    ...
    try {
      await fetch('https://nail-care-api.onrender.com/api/manicure-time', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        // 載入每日任務
        const dailyResponse = await fetch('https://nail-care-api.onrender.com/api/daily-tasks');
        if (dailyResponse.ok) {
        ...
        // 載入每週任務
        const weeklyResponse = await fetch('https://nail-care-api.onrender.com/api/weekly-tasks');
        ...
        // 載入美甲時間
        const manicureResponse = await fetch('https://nail-care-api.onrender.com/api/manicure-time');
